...
This skill calls tools on the `wood-fired-tasks` MCP server. The doc uses shorthand `wood-fired-tasks:<tool>`; harness tool names are `mcp__wood-fired-tasks__<tool>`. If a call returns `InputValidationError`, load via `ToolSearch` (`select:mcp__wood-fired-tasks__add_comment`) and retry.
...
4. **Pre-flight task existence check**
   - Call `wood-fired-tasks:get_task` with `id=<task-id>` (cheap; gives a clean "task not found" error before mutating).
...
5. **Idempotency check** (avoid double-comment from accidental retry)
   - Call `wood-fired-tasks:get_comments` with `task_id=<id>`. If the most recent comment matches `(author=$AUTHOR, content=<your text>)` AND was created within the last 60s, warn: "Identical comment from <author> exists at <created_at> — append anyway? (y/N)". Default abort on N or no response.
...
7. **Confirm completion**
   ...
   - On error: report the server message AND suggest a retry path (e.g. "rerun once after checking ToolSearch loaded `mcp__wood-fired-tasks__add_comment`").
