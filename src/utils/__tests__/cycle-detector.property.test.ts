...
describe('CycleDetector property tests', () => {
  test.prop([nodeArb, nodeArb])('empty graph has no cycles for distinct nodes', (a, b) => {
    fc.pre(a !== b);
    const detector = new CycleDetector([]);
    return detector.wouldCreateCycle(a, b) === false;
  });

  test.prop([nodeArb])('self-loop is always a cycle', (a) => {
    const detector = new CycleDetector([]);
  ...
  test.prop([nodeArb, nodeArb])('mutual edge creates a cycle', (a, b) => {
    fc.pre(a !== b);
