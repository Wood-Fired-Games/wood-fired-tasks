...
const callbackRoute: FastifyPluginAsync<AuthRoutesOptions> = async (fastify, opts) => {
  fastify.get('/callback', { config: { skipAuth: true } }, async (request, reply) => {
    ...
    try {
      ...
      request.log.error({ err, requestId: request.id, peerIp: request.ip }, 'oidc.callback_failed');
      request.session.set('oidc.handshake', undefined);
      return reply
        .header('Cache-Control', 'no-store')
    ...
    if (!claims) {
      ...
      request.log.error({ requestId: request.id, peerIp: request.ip }, 'oidc.id_token_missing');
      request.session.set('oidc.handshake', undefined);
      return reply
        .header('Cache-Control', 'no-store')
        .redirect('/auth/error?reason=exchange_failed', 302);
    ...
    const replyWithRedirect = reply.header('Cache-Control', 'no-store').redirect(redirectTo, 302);
    ...
    const setCookieList: string[] = Array.isArray(setCookieRaw)
      ...
      : setCookieRaw !== undefined && setCookieRaw !== null
        ? [String(setCookieRaw)]
        : [];
    const sessionCookieLine = setCookieList.find((c) => c.startsWith(`${opts.sessionCookieName}=`));
