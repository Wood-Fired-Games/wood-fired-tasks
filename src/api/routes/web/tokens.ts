...
const tokensWeb: FastifyPluginAsync = async (fastify) => {
  fastify.get('/me/tokens', { config: { skipAuth: true } }, async (request, reply) => {
    ...
    if (!sessionUser) {
      return reply.header('Cache-Control', 'no-store').redirect('/auth/login?next=/me/tokens', 302);
    }
    ...
    const mintedToken = getFlashAndClear<'mintedToken'>(request, 'mintedToken');
    return reply
  ...
  fastify.post('/me/tokens/:id/revoke', { config: { skipAuth: true } }, async (request, reply) => {
    ...
    return reply.header('Cache-Control', 'no-store').redirect('/me/tokens', 303);
  });
};
