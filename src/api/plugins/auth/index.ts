 ...
 */
function applyPrincipal(request: FastifyRequest, result: AuthResult, apiKeyLabel?: string): void {
 ...
 */
function enforceSessionOnly(request: FastifyRequest, reply: FastifyReply): boolean {
  if (request.routeOptions.config.sessionOnly === true && request.authMethod !== 'session') {
    reply.code(403).send({
 ...
 */
function sendInternalError(request: FastifyRequest, reply: FastifyReply, err: unknown): void {
