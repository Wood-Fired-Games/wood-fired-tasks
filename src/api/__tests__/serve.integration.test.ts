...
// Both tests move cwd outside the repo (process.chdir) to prove serve does not
// depend on cwd for the DB path. process.chdir() throws inside worker_threads,
// and Stryker's vitest runner forces pool:'threads' for its mutation dry run
// (task #823) — so this integration suite skips there and runs fully under
// normal `npm test` (forks pool / main thread), preserving its CI value.
describe.skipIf(!isMainThread)(
  'Task #733 — serve boots from any cwd, migrates app-data DB, /health 200',
  () => {
    let tmpHome: string;
