...
export interface IApiTokenRepository {
   ...
   * `'[]'`; `expiresAt` defaults to NULL; `projectId` — the optional project
   * binding from Security Audit finding M1 / task #1635 — defaults to NULL,
   * which means "unbound / every project" (the pre-#1635 behaviour).
   *
   * Throws on FK violation (`userId` references a non-existent user, or
   * `projectId` references a non-existent project), on `InvalidScopeError`
   * (a scope outside the canonical taxonomy) and on
   * `InvalidProjectBindingError` (a binding that is not a positive integer).
   */
  insert(input: {
    userId: number;
    name: string;
