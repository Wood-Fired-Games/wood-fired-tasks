...
> ## ✨ New in v1.16 — WSJF Prioritization
> Rank your backlog by **economic value**, not a flat `priority` field most teams leave at "all high." **Variance-enforced column anchoring** forces a usable ordering out of a degenerate backlog, and **propagation-adjusted effective WSJF** lets your dependency graph surface the modest prerequisite that unblocks the most downstream value — so `/tasks:loop` and `/tasks:loop-dag` drain the highest value-per-effort work first. A workflow most task trackers don't offer; opt-in and fully backward-compatible. **→ [See WSJF Prioritization](#wsjf-prioritization).**
