...
**Resolve a real identity** before any `assignee` (on `claim_task`) or `author` (on `add_comment`) field — do NOT pass the literal `"user"` (that destroys cross-machine audit attribution). In priority order: (1) the SCM-configured author email (the backend's configured author identity), (2) `$USER`, (3) `claude-<model>-<purpose>` (e.g. `claude-opus-4.7-loop-dag`). Pick once at top of invocation and capture as `$ASSIGNEE` (used for both `assignee` and `author` throughout this run). Detailed enforcement rules already embedded in the per-worker brief / claim / comment sections below (and reused from `loop.md` / `loop-shared.md`) — this block is the canonical pointer.
...
- `verdict: "PASS"` → commit the worker's changes if not already committed (mirrors `loop.md` §Step 6 — Commit + push). Because loop-dag workers run in ISOLATED worktrees (§3b), their changes must be integrated onto the integration tree ONE task at a time — confirm each recorded change holds only that task's files before integrating the next; never batch dependent patch-integration steps. Full mechanics (shared-file slice recipe + kill-safe re-slice): [`loop-shared.md` §Q](./loop-shared.md#q-worktree-patch-integration-mechanics-loop-dag-run-end--per-wave). Then call `wood-fired-tasks:update_task with updates={ "status": "done", "verification_evidence": <full evidence> }`. Add the close-out comment per `loop.md` §Step 8 template.
...
BROKEN integration — bisect the wave's per-task recorded changes
(stash-free: re-run the failing test at each of the wave's recorded changes) to
...
- **`## Retained Worktrees`** — emitted by the §5g teardown: one bullet per `worktree-agent-*` branch it did NOT remove (un-integrated work), as `` `<path>` (branch `<branch>`) — <n> un-integrated patch(es); inspect with `tasks scm changed-files <base>` (run within that isolation) ``. Sentinel `_No retained worktrees — all run worktrees were fully integrated and removed._` on a clean run; `_No worktrees created (no isolated workers dispatched)._` when no `isolation: "worktree"` workers ran.
...
Same rationale as `loop.md` §9e — `.planning/` is gitignored per project policy. LOOP-RUN.md and the per-wave / run-termination integration-audit artifacts are local-machine per-run audit trails, not versioned artifacts. The orchestrator MUST NOT `tasks scm stage` any `.planning/loops/` artifact, nor modify `.gitignore`.
...
Terminal step — runs ONCE after §5f's emit, on EVERY termination path — to reclaim the `isolation: "worktree"` worker worktrees (§3b) + `worktree-agent-*` branches the harness never auto-removes (a committed/edited worktree is "changed", so it is never auto-cleaned and they pile up across runs). Discovery-based and integration-gated so it can ONLY ever delete fully-integrated leftovers: enumerate the isolation ids the run dispatched (plus any still-present isolation the backend/harness reports for the same `--context`); for each, `tasks scm teardown-isolation <id>` performs the backend-appropriate integration-safety gate against the run's integration baseline (`tasks scm baseline`, `data.id`, usually the tip of `main`) and reclaims ONLY fully-integrated isolation — for a git-backend the platform harness performs the concrete reclamation (unlock the locked isolation, force-remove the "changed" tree, delete the reclaimed branch, prune stale admin entries). Any isolation still holding an un-integrated change is RETAINED and surfaced in the `## Retained Worktrees` LOOP-RUN.md block (§5d), then LOOP-RUN.md is re-emitted (§5b). Idempotent / kill-safe. **Full procedure + the `/tasks:loop` not-affected rationale: [loop-shared.md §N](loop-shared.md#n-worktree-teardown-loop-dag-run-end).**
...
- **One commit per task, not per wave.** Each worker's PASS triggers its own commit (mirrors `loop.md` §Step 6). Wave granularity is the dispatch + audit unit; commit granularity is per task. This keeps the change history readable and per-task revertible.
...
|---------|----------------|---------------|-------------------|
| **PASS** | `update_task → status=done`, write evidence | `tasks scm stage` + `tasks scm record` + `tasks scm publish` | Downstream becomes frontier-eligible. |
