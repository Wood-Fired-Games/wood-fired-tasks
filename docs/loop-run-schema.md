...
| `execution_model` | string (optional) | non-empty model id or `auto` | Configurable Task Models (#924). The model ref the run forced for worker (`execution`-role) dispatches via the `--execution-model` run-arg override (see `skills/tasks/loop-shared.md` §R). **Omitted entirely when unset** — absence means the run used per-project `resolve_model` resolution rather than a forced override. Pure provenance for replay. | `claude-sonnet-4-20250514` |
| `validation_model` | string (optional) | non-empty model id or `auto` | Configurable Task Models (#924). The model ref the run forced for verifier (`validation`-role) dispatches via `--validation-model` (see §R). Omitted when unset. | `auto` |
| `planning_model` | string (optional) | non-empty model id or `auto` | Configurable Task Models (#924). The model ref the run forced for planning-role dispatches via `--planning-model` (see §R). Omitted when unset. | `claude-opus-4-20250514` |
